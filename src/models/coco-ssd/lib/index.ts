...
export class ObjectDetection {
  ...
  private async infer(
    ...
    const height = batched.shape[1] as number;
    const width = batched.shape[2] as number;
    ...
    const [maxScores, classes] =
        this.calculateMaxScores(scores, result[0].shape[1] || 0, result[0].shape[2] || 0);
    ...
    const indexTensor = tf.tidy(() => {
      const boxes2 =
          tf.tensor2d(boxes, [result[1].shape[1] || 0, result[1].shape[3] || 0]);
