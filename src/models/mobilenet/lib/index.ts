...
class MobileNetImpl implements MobileNet {
  model: tfconv.GraphModel | undefined;
  ...
  async load() {
    ...
    const result = tf.tidy(
                       () => this.model && this.model.predict(tf.zeros(
  ...
  infer(
    ...
    return tf.tidy(() => {
      ...
      if (embedding) {
        ...
        const internal =
        this.model && this.model.execute(batched, embeddingName) as tf.Tensor4D;
        result = tf.squeeze(internal || [], [1, 2]);
      } else {
        const logits1001 = this.model && this.model.predict(batched) as tf.Tensor2D;
